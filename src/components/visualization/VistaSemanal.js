...
import { Play, Edit, Trash2, Check, PenTool, X, Trophy } from "lucide-react";
...
import ResumenHoras from "./ResumenHoras.js";
...
function VistaSemanal({
  ...
  const [mostrarTopUsuarios, setMostrarTopUsuarios] = useState(false);
        ...
        <div className="backdrop-blur-md bg-black/20 border border-white/10 rounded-xl p-4 shadow-lg mb-6">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-3">
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={() => setEliminarAlReiniciar?.(!eliminarAlReiniciar)}
                title={eliminarAlReiniciar ? "Al cambiar de semana se eliminarán las tareas" : "Al cambiar de semana se reiniciarán las tareas"}
                className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border transition-colors duration-200 ${
                  eliminarAlReiniciar
                    ? "bg-red-500/20 border-red-500/40 text-red-400 hover:bg-red-500/30"
                    : "bg-white/5 border-white/10 text-white/40 hover:bg-white/10"
                }`}
              >
                <span className={`w-1.5 h-1.5 rounded-full ${eliminarAlReiniciar ? "bg-red-400" : "bg-white/30"}`} />
                {eliminarAlReiniciar ? "Vaciar al reiniciar" : "Reiniciar completadas"}
              </button>
            </div>
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="overflow-x-auto">
              ...
              {/* Resumen de horas — solo desktop */}
              <div className="hidden md:block">
                <ResumenHoras
                  tiempoPlanificado={calcularHorasTotales()}
                  tiempoLibre={calcularHorasRestantes()}
                />
              </div>
                ...
                {tareasSemanaFiltradas.length > 0 ? (
                  <div className="space-y-3">
                    {tareasSemanaFiltradas.map((tarea) => (
                      <div
                        key={tarea.id}
                        className="backdrop-blur-md bg-black/20 border border-white/10 rounded-xl p-4 shadow-lg"
                      >
              ...
              {/* Solo móvil — debajo de las tareas */}
              <div className="md:hidden mt-4 space-y-2">
                ...
                <button
                  onClick={() => setMostrarTopUsuarios(!mostrarTopUsuarios)}
                ...
                >
                  ...
                  {mostrarTopUsuarios ? "Ocultar top usuarios" : "Mostrar top usuarios"}
                </button>
                {mostrarTopUsuarios && <TopUsuarios usuarios={usuarios} />}
              </div>
            </div>
          </div>
        </div>
