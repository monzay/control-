import { Menu, X, LogIn, UserPlus, EyeOff, Eye, Flame, Zap } from "lucide-react"
  ...
  return (
    <header className="sticky top-0 z-30 bg-black/30 backdrop-filter backdrop-blur-md border-b border-white/5 py-3 px-4 flex items-center justify-between">
...

      {
        ...
        <button
          onClick={() => setModoFocus?.(!modoFocus)}
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm transition-colors duration-200 ${
            modoFocus
              ? "bg-emerald-600 text-white"
              : "bg-white/5 hover:bg-white/10 text-white/60 hover:text-white"
          }`}
          title={modoFocus ? "Salir del modo focus" : "Activar modo focus"}
        >
          <Zap className="h-4 w-4" />
          <span className="hidden md:inline">Modo Focus</span>
        </button>
        <button
