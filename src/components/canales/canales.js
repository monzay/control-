...
import CanalNotas from './CanalNotas';
import CanalObjetivos from './CanalObjetivos';
import CanalMiembros from './CanalMiembros';
...
const Canales = () => {
  ...
  const [channelMembers, setChannelMembers] = useState([]); // Nuevo estado para los miembros del canal
  const [currentUserId, setCurrentUserId] = useState(null); // Asumiendo que obtendremos el ID del usuario actual de alguna manera

  useEffect(() => {
    refrescarCanales();
    refrescarInvitacionesPendientes();
    // Aquí deberías obtener el ID del usuario actual. Por ahora, un placeholder.
    // Podrías obtenerlo del contexto de autenticación, o de un endpoint de usuario.
    setCurrentUserId(/* Lógica para obtener el ID del usuario actual */ 1); // Reemplaza 1 con la lógica real
  }, [refrescarCanales, refrescarInvitacionesPendientes]);

  const manejarAceptarInvitacion = async (canalId) => {
    await aceptarInvitacionCanal(canalId);
    setShowInvitations(false);
  };

  const manejarRechazarInvitacion = async (canalId) => {
  ...
  const manejarSeleccionarCanal = async (channel) => {
    setSelectedChannel(channel);
    // Cargar miembros del canal cuando se selecciona
    const members = await listarMiembrosCanal(channel.id);
    setChannelMembers(members);
  };

  const manejarVolverACanales = () => {
    setSelectedChannel(null);
    setChannelMembers([]); // Limpiar miembros al volver
  };
  ...
  if (error) {
    ...
    <div className="p-4 bg-black/50 rounded-lg shadow-lg text-white">
      {!selectedChannel ? (
        <>
          <h1 className="text-2xl font-bold mb-6">Sección de Canales</h1>
                      ...
                      {invitacionesPendientes.map((invitacion) => (
                        <li key={invitacion.id} className="px-4 py-3">
                          ...
                          <div className="mt-3 flex justify-end space-x-2">
                            <button
                              onClick={() => manejarAceptarInvitacion(invitacion.canal.id)}
                            ...
                            <button
                              onClick={() => manejarRechazarInvitacion(invitacion.canal.id)}
            ...
            {canales && canales.length > 0 ? (
              <ul className="space-y-3">
                {canales.map((miembroCanal) => (
                  <li 
                    key={miembroCanal.canal.id} 
                    className="bg-gray-700 p-4 rounded-lg border border-gray-600 flex justify-between items-center cursor-pointer hover:bg-gray-600 transition-colors duration-200"
                    onClick={() => manejarSeleccionarCanal(miembroCanal.canal)}
                  >
        <div>
        ...
        <div className="transition-all duration-300 ease-in-out">
          <div className="flex items-center mb-6">
            <button
              onClick={manejarVolverACanales}
              className="bg-gray-700 hover:bg-gray-600 text-white font-bold py-2 px-4 rounded-lg flex items-center transition-colors duration-200 mr-4"
            >
              <ArrowLeft className="h-5 w-5 mr-2" />
              Volver a Canales
            </button>
            <h1 className="text-2xl font-bold">Canal: {selectedChannel.nombre}</h1>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <CanalNotas canalId={selectedChannel.id} />
            <CanalObjetivos canalId={selectedChannel.id} />
            <CanalMiembros canalId={selectedChannel.id} miembros={channelMembers} creadorId={selectedChannel.creadorId} />
          </div>
        </div>
      )}
    </div>
...
export default function EnvoltorioCanales() {
  return (
    <CanalProvider>
      <Canales />
    </CanalProvider>
